...
|----------|-------------|-----------|
| `CREDIHOME_API_KEY` | Não | Token adicional fornecido pela Credihome. Quando informado, é enviado no header configurável (padrão `Authorization`) e no header auxiliar `x-api-key`. |
